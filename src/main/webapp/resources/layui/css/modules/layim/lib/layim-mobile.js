...
layui.define(['laytpl', 'upload', 'layer-mobile', 'zepto'], function(exports){
  ...
  var tempvoiceTime=null;
  var tempVoiceMsg=null;
  var topChat=null;
  ...
  //转发消息
  LAYIM.prototype.TransferMessage = function(){
    return ChatlogTransfer(), this;
  };
  //添加好友/群
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layim-chat-send"><button class="layim-send layui-icon layui-icon-mike" id="voiceInput" ></button><input type="text" autocomplete="off"><button class="layim-send layui-disabled" layim-event="send">发送</button></div>'
  ...
  layui.data.tempVoiceMsg= function(voicecontent,voiceTime){
    tempVoiceMsg = voicecontent;
    tempvoiceTime = voiceTime;
  }
  layui.data.sendVoiceMsg= function(type){
    sendMessage("voiceMsg");
  }
  layui.data.sendHistory= function(data){
    sendMessage("history",data);
  }
  ...
  layui.data.layimChat= function() {
    console.info(topChat)
    return topChat;
  }
  //转换内容
  layui.data.content = function(content){
    ...
    .replace(/audio\[([^\s]+?)\]/g, function(audio){  //转义音频
      return '<div class="layui-unselect layui-layim-audio" layim-event="playAudio" data-src="' + audio.replace(/(^audio\[)|(\]$)/g, '') + '"><i class="layui-icon">&#xe688;</i><p>'+tempvoiceTime+'s</p></div>';
    })
  ...
  var elemChatMain = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">'
    ,'<div class="layim-chat-user"><img src="{{ d.avatar || layui.cache.layimAssetsPath + \'images/default.png\' }}" alt="{{ d.uid || d.id }}"><cite>'
      ,'{{ d.username||"佚名" }}'
    ,'</cite></div>'
  ...
  var thisChat = function(){
    ...
    return {
      elem: cont
      ,data: to
      ,textarea: cont.find('input[type="text"]')
    };
  };
  ...
  //发送消息
  var sendMessage = function(type,historyTmp){
    var data = {
    ...
    if(type=="voiceMsg"){
      if(tempVoiceMsg==null){
        ...
        var testcontent="audio[https://fileserver.jwjxinfo.com//statics/chat_audio/qf1619592229969.mp3]";
        data.content = tempVoiceMsg;
        tempVoiceMsg=null;
      }
    }else if(type=="history"){
      data.content = historyTmp.fromContent;
      time = historyTmp.timestamp;
      console.info(historyTmp);
    }else{
  ...
  var elemChatMain3 = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">'
    ,'<div class="layim-chat-user"><img src="{{ d.fromavatar || layui.cache.layimAssetsPath + \'images/default.png\' }}" alt="{{ d.uid || d.id }}">'
    ,'<cite>{{layui.data.date(d.timestamp)}}&nbsp;{{ d.fromUsername||"佚名" }}</cite>'
    ,'</div>'
    ,'<div class="layim-chat-text">{{ layui.data.content(d.fromContent||"&nbsp;") }}</div>'
    ,'</li>'].join('');
  ...
  var viewChatlog = function(){
    ...
    //     sendMessage.time = index.timestamp;
    //   }
    //   item.index = index
    ...
    var ul = thatChat.elem.find('.layim-chat-main ul');
     ...
     var jsonData={};
     jsonData["mid"] =cache.mine.id;
     jsonData["toid"] =thatChat.data.id;
     jsonData["limit"] =50;
     jsonData["curr"] =1;
    ...
    $.ajax({
      ...
      success: function (result) {
        if (result.code == 0 ) {
          var chatWindows = result.data;
          $.each(chatWindows,function(index,item) {
            if(item.fromId==cache.mine.id){
              item.mine=true;
            }else{
              item.mine=false;
            }
            ...
            // }
            item.index = index
            ul.append(laytpl(elemChatMain3).render(item));
          })
          chatListMore();
  ...
  /**
   * YYJ 2021/5/10  转发列表所有聊天信息选择 checkbox
   */


  var ChatlogTransfer = function(){
    $("body .chooseChat").toggle();
    // $("body .chooseChat").css("display","block");
    //
    // var local = layui.data('layim-mobile')[cache.mine.id] || {};
    // var thatChat = thisChat(), chatlog = local.chatlog || {};
    // var ul = thatChat.elem.find('.layim-chat-main ul');
  ...
  // ,'<div class="layim-chat-checkbox chooseChat" ><input type="checkbox" value="{{d.index}}"  lay-filter="chatList" lay-skin="primary" /></div>\n'

  var elemChatMain2 = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">'
    ,'<div class="layim-chat-user"><img src="{{ d.avatar || layui.cache.layimAssetsPath + \'images/default.png\' }}" alt="{{ d.uid || d.id }}"><cite>'
    ,'{{layui.data.date(d.timestamp)}}&nbsp;{{ d.username||"佚名" }}'
    ,'</cite></div>'
    ,'<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp;") }}</div>'
    ,'</li>'].join('');
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,back: function(othis){
      ...
      // var str='<p style="font-size: 16px;">深圳经纬捷讯在线客服<span class="layim-chat-status"></span></p>';
    ...
    ,image: function(othis){
      ...
      layui.upload.render({
        ...
        ,before: function(obj){ //obj参数包含的信息，跟 choose回调完全一致，可参见上文。
          // loadingIndex = layer.msg('正在上传', {icon: 16});
        }
        ,done: function(res){
          ...
          if(res.code == 0){
            ...
            if(type === 'images'){
              ...
              if (res.data.fileType === 'video') {
                focusInsert(thatChat.textarea[0], 'video[' + (res.data.src || '') + ']');
              } else {
    ...
    ,playVideo: function(othis){
      ...
      events.playVideo.index = layer.open({
        ...
        ,content:'<div id="mui-player"></div>'
            ...
            // '    width="640"\n' +
            // '    height="264"\n' +
            // '    data-setup="{}" style="position: absolute; width: 100%; height: 100%;" ' +
            // 'src="'+ videoData +'" autoplay="autoplay"></video></div>'
      });
    ...
    ,about: function(){
      layer.open({
        content: '<p style="padding-bottom: 5px;">深圳市经纬捷讯信息技术有限公司</p>' +
            '<p style="padding-bottom: 5px;">提供支持</p>'
...
}).link(layui.cache.layimAssetsPath + 'mobile.css', 'skinlayim-mobilecss') //加载 css;;